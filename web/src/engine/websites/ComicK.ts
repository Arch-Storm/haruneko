...
export default class extends DecoratableMangaScraper {
  ...
  public override async FetchManga(
    ...
    const manga = await FetchWindowScript<APIManga>(
      new Request(url),
      "__NEXT_DATA__.props.pageProps.comic",
      2000
    );
  ...
  private async GetMangasFromPage(
    page: number,
    provider: MangaPlugin
  ): Promise<Manga[]> {
    try {
      const data = await FetchJSON<APIManga[]>(
        new Request(
          new URL(
            `v1.0/search?page=${page}&limit=50&sort=user_follow_count`,
            this.apiUrl
          )
        )
      );
      return data.map(
        (item) => new Manga(this, provider, item.hid, item.title.trim())
      );
    } catch {
      // TODO: Do not return empty list for generic errors
      return [];
    }
  }
  ...
  private async GetChaptersFromPage(
    manga: Manga,
    page: number
  ): Promise<Chapter[]> {
    const { chapters: entries } = await FetchJSON<APIChapters>(
      new Request(
        new URL(
          `/comic/${manga.Identifier}/chapters?page=${page}`,
          this.apiUrl
        ),
        {
          headers: {
    ...
    return entries.map((item) => {
      ...
      return new Chapter(
        ...
        item.hid,
        title,
        ...(chapterLanguageMap.has(item.lang)
          ? [chapterLanguageMap.get(item.lang)]
          : [])
      );
    });
  }
  ...
  public override async FetchPages(chapter: Chapter): Promise<Page[]> {
    const {
      chapter: { md_images },
    } = await FetchJSON<APISingleChapter>(
      new Request(new URL(`/chapter/${chapter.Identifier}`, this.apiUrl), {
        headers: {
          Referer: this.URI.href,
          Origin: this.URI.origin,
        },
    ...
    return md_images.map(
      (image) =>
        new Page<PageParameters>(
          this,
          chapter,
          new URL(image.b2key, `https://s3.comick.ink/comick/`),
          {
            mirrors: [
